...
phyl_syncom_pruned %>%
  plot_net() +
  ggtitle("Bray curtis distances between samples")
```
...
Sometimes, you have isolates that are only in one of your samples, I suggest you try to filter out anything that you in total find less than a 100 times across the 5 replicates
...
Compare those edge-sets and collect which are shared, differ in sign, or are unique for one. If you dont understand what this does, check the documentation for case_when.
